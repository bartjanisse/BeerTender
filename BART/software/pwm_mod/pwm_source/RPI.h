...
#define GPIO_SET(g) 			*(gpio_addr + ((g)/32) + GPSET0)  = (1 << (g)) // Set GPIO bit 0..31, 32..53
#define GPIO_CLR(g) 			*(gpio_addr + ((g)/32) + GPCLR0)  = (1 << (g)) // Clear GPIO bit 0..31, 32..53
#define GPIO_READ(g) 			*(gpio_addr + ((g)/32) + GPLEV0) &= (1 << (g)) // Read GPIO input 0..31, 32..53
 
volatile unsigned int *gpio_addr;
volatile unsigned int *pwm_addr;
volatile unsigned int *clk_addr;
